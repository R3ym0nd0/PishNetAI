# AI Service

This Flask microservice receives phishing feature data from the Node.js backend and returns a machine learning prediction.

## Endpoint

- `POST /api/predict`

## Input

The service expects these numeric features:

- `uses_https`
- `has_ip_in_url`
- `subdomain_count`
- `url_length`
- `hostname_length`
- `path_length`
- `query_length`
- `path_depth`
- `query_param_count`
- `encoded_path_count`
- `encoded_query_count`
- `suspicious_path_keyword_hits`
- `suspicious_query_keyword_hits`
- `sensitive_query_param_hits`
- `dot_count`
- `hyphen_count`
- `digit_count`
- `special_char_count`
- `has_at_symbol`
- `has_double_slash_in_path`
- `has_shortener_domain`
- `has_punycode`
- `suspicious_tld`
- `form_count`
- `input_count`
- `password_field_count`
- `hidden_input_count`
- `has_external_form_action`
- `iframe_count`
- `external_link_count`
- `external_script_count`
- `keyword_login`
- `keyword_verify`
- `keyword_account`
- `keyword_secure`
- `keyword_update`
- `keyword_password`
- `keyword_urgent`
- `keyword_confirm`
- `keyword_signin`
- `suspicious_keyword_hits`
- `redirect_count`
- `final_domain_differs_from_input`
- `redirected`

## Output

```json
{
  "prediction": 0,
  "confidence": 0.93
}
```

Where:

- `0` = Safe
- `1` = Phishing
